;(function ( $, window, undefined ) {
  ...
  var flag = 1;
    ...
    jQuery(document).on('iframe_load',function(e,data){
        ...
        jQuery('.cp-cust-form .smile-input', window.parent.document).on( 'change', function( event ) {
...
function hide_butn_style(){
    ...
    if( btn_attached_email == '1' && layout == 'cp-form-layout-3' && flag !== 1 ){
        ...
        if( option == 'cp-btn-3d' || option == 'cp-btn-outline' ){
            jQuery('select[name^="btn_style"] option[value="cp-btn-flat"]').prop("selected","selected");
            btn_shadow.addClass('display-shadow');
        }else{
            btn_shadow.removeClass('display-shadow');
            jQuery('select[name^="btn_style"] option[value="'+option+'"]').prop("selected","selected");
        }
    }else{
        btn_shadow.removeClass('display-shadow');
        jQuery('select[name^="btn_style"] option[value="'+option+'"]').prop("selected","selected");
